...
public class QuestionnaireResourceIT {
  ...
  public void setup() {
    ...
    final QuestionnaireResource questionnaireResource =
        new QuestionnaireResource(questionnaireRepository, userRepository, feignUserClient);
    this.restQuestionnaireMockMvc =
        MockMvcBuilders.standaloneSetup(questionnaireResource)
  ...
  public void getAllQuestionnaires() throws Exception {
    ...
    restQuestionnaireMockMvc
        .perform(get("/api/questionnaires?sort=id,desc"))
        ...
        .andExpect(
            jsonPath("$.[*].constructionYearMin").value(hasItem(DEFAULT_CONSTRUCTION_YEAR_MIN)))
        .andExpect(
            jsonPath("$.[*].constructionYearMax").value(hasItem(DEFAULT_CONSTRUCTION_YEAR_MAX)))
        .andExpect(jsonPath("$.[*].type").value(hasItem(DEFAULT_TYPE.toString())))
        .andExpect(
            jsonPath("$.[*].totalCostMin").value(hasItem(DEFAULT_TOTAL_COST_MIN.doubleValue())))
        .andExpect(
            jsonPath("$.[*].totalCostMax").value(hasItem(DEFAULT_TOTAL_COST_MAX.doubleValue())));
  }
  ...
  public void getQuestionnaire() throws Exception {
    ...
    restQuestionnaireMockMvc
        .perform(get("/api/questionnaires/{id}", questionnaire.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingQuestionnaire() throws Exception {
    // Get the questionnaire
    restQuestionnaireMockMvc
        .perform(get("/api/questionnaires/{id}", Long.MAX_VALUE))
  ...
  public void updateQuestionnaire() throws Exception {
    ...
    Questionnaire updatedQuestionnaire =
        questionnaireRepository.findById(questionnaire.getId()).get();
    // Disconnect from session so that the updates on updatedQuestionnaire are not directly saved in
    // db
  ...
  public void deleteQuestionnaire() throws Exception {
    ...
    restQuestionnaireMockMvc
        .perform(
            delete("/api/questionnaires/{id}", questionnaire.getId())
